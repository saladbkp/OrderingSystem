...
import com.runnerdao.ViewReviewDao;
...
public class ReviewView extends JPanel{
	...
	int HEIGHT = 150;
        ...
        String columns[] = {"Order ID","Date","Review","Method"};
        JTable tasktable = null;
        JScrollPane jscrollpane;
        DefaultTableModel model;
        TableColumnModel columnModel;
	ViewReviewDao viewhistory= new ViewReviewDao(); 
        Style style = new Style();
        
	public ReviewView (int x,int y, int width, int height) {
		// separate 2 windows ?????? 
		...
		jpanel1.setBackground(Color.YELLOW);
                ...
                // add button sample
                JButton additembutton = new JButton("Delete");
		
		JLabel reviewlabel = new JLabel("Review ");
		reviewlabel.setFont(style.title);
                
                jpanel1.add(reviewlabel);
		...
		this.add(jpanel1);
                table();
                model = Tools.addDataTable(viewhistory.findTaskData(), model);
